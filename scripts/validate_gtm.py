...
def _yaml_parse_diagnostic(block: str, key: str, exc: Exception) -> str:
    ...
    scalar field (``as_of``, a ``next_actions[]`` item) as an UNQUOTED YAML scalar; if
    that value contains a ``: `` (colon-space), ``yaml.safe_load`` reads it as a nested
    mapping and throws ``mapping values are not allowed here``. Before F9.1 the block
    then reported as the GENERIC "no parseable block", masking the real cause for ~24h.

    This surfaces the parser's line/column + the offending source line, and — for the
    classic unquoted colon-space scalar — names the field and prescribes single-quoting.
    """
    ...
    if mark is not None and 0 <= mark.line < len(lines):
        loc = f" at line {mark.line + 1}, col {mark.column + 1}"
        src = lines[mark.line].strip()
        mkey = re.match(r"\s*-?\s*([A-Za-z0-9_]+)\s*:", lines[mark.line])
        if mkey:
            field = mkey.group(1)
    # "mapping values are not allowed here" is the signature of an unquoted scalar whose
    # value itself contains a `: `, mis-parsed as a nested mapping. Also catch the case
    # where the offending line's value visibly carries a colon-space.
    value_has_colon_space = bool(src and ": " in src.split(":", 1)[-1])
    hint = ""
    if "mapping values are not allowed here" in str(problem) or value_has_colon_space:
...
def _yaml_block(path: Path, key: str):
    ...
    for m in re.findall(r"```yaml\n(.*?)```", path.read_text(), re.S):
        try:
            ...
            if parse_error is None and top_key_re.search(m):
                parse_error = _yaml_parse_diagnostic(m, key, e)
            continue

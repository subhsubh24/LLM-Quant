...
## 2026-07-25 — Research Run 30: executed the Run 24 pre-registered EXP-009 step 0 (Kalshi employment reliability decomposition, own-environment egress open, real data) + a fresh external-literature sweep surfacing one strong new primary source directly on B8's thesis. RECOMMEND-only, no code shipped, no ROADMAP steer.

- **Orientation.** Read RESEARCH_MEMORY.md / RESEARCH_PLAYBOOK.md / ROADMAP.md / VISION.md / docs/BUSINESS_CASE.md / GROWTH_STATUS.md first. `git log` HEAD is `e066927` (2026-07-24 quality-grade run, #415 — a QUALITY_SCORECARD pass, not alpha work); the two prior alpha commits are #410 (EXP-010 cost realism) and #411 (B8 step iii semantic classifier), both 2026-07-24. Binding constraint unchanged since 07-24: both real-money mechanisms tested to date (bucket-calibration EXP-002/003/005, now cost-model-robust-refuted; EXP-006 fade-the-spike) stay REFUTED; B8 is blocked on step (ii) (egress-gated historical co-listed corpus); EXP-009 is `proposed`, un-tested.
- **Egress check (self-validated, not assumed): open.** Direct `curl` from this run's own environment to `gamma-api.polymarket.com`, `clob.polymarket.com`, `api.elections.kalshi.com`, and `huggingface.co` all returned HTTP 200 — so, per the standing "re-probe before assuming owner-gated" instruction, this run executed a real experiment against unmodified repo modules rather than only reading literature.
- **EXP-009 step 0 — Kalshi KXJOBLESS reliability (Murphy) decomposition. PRE-REGISTERED before running:** series=`KXJOBLESS` (the deepest single clean-market, non-strike-ladder employment series the 2026-07-22/23 factory probes already named), `historical=True` tier (`KalshiHistoryFetcher`, #408, unmodified), `decision_lead=3 days` (midpoint of the 2-5-day usable window the 2026-07-23 factory run measured for this weekly series before N collapses), `n_bins=5`. Script imports ONLY unmodified repo modules (`kalshi_history_fetcher.KalshiHistoryFetcher`, `calibration.reliability_curve`/`brier_score`) — no repo code changed, no strategy fit, no PnL, no gate touched.
  - **First pass hit Kalshi's rate limit:** an un-paced per-market fetch got 429'd on most candlestick calls and only assembled N=29/74 leakage-safe records (reproducing the 2026-07-23 factory run's own N=29 finding almost exactly — Brier 0.271 that run's read was 0.238, both close, both **rate-limit-truncated samples, not the true reachable corpus**). This matters: prior runs (07-22, 07-23) read N=29-71 as the natural ceiling of this series; it is actually a 429-throttling artifact.
  - **Re-ran with a 1.5s inter-request delay + one backoff retry per market (same unmodified `to_historical_market` call, only the calling loop paced) — assembled N=67/74** (7 genuinely skipped: no qualifying pre-resolution tick). This is the load-bearing, higher-fidelity read.
  - **Result (N=67):** base_rate(YES)=0.5224; **crowd_brier=0.2080 < base_rate_brier=0.2495 — the crowd genuinely beats always-guessing the base rate** (the opposite of the 2026-07-23 run's N=29 read, where crowd_brier 0.238 barely beat base-rate 0.250 — the larger, less-throttled sample shows a CLEARER crowd edge over naive, not a weaker one). Murphy decomposition (Brier = reliability − resolution + uncertainty): **reliability (miscalibration) = 0.0164** — SMALL, about 8% of the total Brier; resolution (information content beyond the base rate) = 0.0589; uncertainty (irreducible, base_rate×(1−base_rate)) = 0.2495 — the DOMINANT term, as expected for a near-50/50 weekly economic release. Per-bin table: gaps are mixed-sign, not a monotone favorite/longshot pattern (`[0.2,0.4)` n=12 gap +0.150; `[0.4,0.6)` n=29 gap +0.055; `[0.6,0.8)` n=18 gap **−0.141**; `[0.8,1.0)` n=7 gap +0.231, tiny-n) — no single directional bias dominates.
  - **What this means for EXP-009 (honest, tempering — not yet a refutation, one series only).** EXP-009 was motivated by NBER w34702's (secondary-sourced, magnitude-unverified) claim that Kalshi employment/labor markets show "the weakest calibration" and "significant systematic mispricing." A raw elevated Brier (the 2026-07-23 run's headline number) CANNOT by itself distinguish genuine miscalibration from irreducible outcome noise — that ambiguity is exactly why Run 24 filed this decomposition as EXP-009's mandatory step 0, before building any predictor. The answer on THIS series: the reliability term is small (0.0164) and the crowd already beats a naive base-rate guess — i.e., on `KXJOBLESS` specifically, most of the elevated Brier IS irreducible weekly-release uncertainty, not crowd miscalibration. This tempers the case for building a `CalibrationBucketStrategy`/`RecencyWeightedBucketModel` fit against THIS series (there is little reliability-term headroom to extract) — it does NOT close EXP-009 outright, since the NBER paper's strongest claim was about unemployment-RATE contracts (`KXECONSTATU3`/monthly strike-ladders), not weekly initial-claims threshold markets, and those series have not been run through this same decomposition (they need release-event clustering first, per Run 22's standing correlated-strike-ladder caution — a separate, not-yet-built step).
  - **Self-validation / fail-closed disclosure:** the corpus is NOT frozen or committed (a re-run today, tomorrow, or at a different pacing can return a different N/read — the 29-vs-67 gap in this run alone proves that); this is a live-data diagnostic, reported honestly as such, not a committed reproducible artifact. The known fidelity caveat from #408/#409 (a one-sided illiquid book's recorded "crowd price" falls back to the ask, biased high) is inherited unchanged. N=67 independent weekly release-events clears the reliability-decomposition's own bar for a diagnostic read but is still short of EXP-009's own pre-registered ≥100-event floor for any significance-gated backtest.
- **External literature sweep (3 WebSearch + 5 WebFetch passes, primary sources direct-fetched where possible, not snippet-only).**
  - **Genuinely new, on-thesis primary source: arXiv:2601.01706, "Semantic Non-Fungibility and Violations of the Law of One Price in Prediction Markets" (Gebele & Matthes).** WebFetched the arXiv abstract page directly (full verbatim abstract, not a search snippet). Builds "the first human-validated, cross-platform dataset of aligned prediction markets," covering **>100,000 events across TEN major venues, 2018–2025**; ~6% of events are concurrently listed across platforms; semantically-equivalent markets show **persistent EXECUTION-AWARE price deviations of 2–4% on average**, attributed to structural friction (semantic non-fungibility / fragmented event identity) rather than informational disagreement. This is the closest external academic evidence yet found directly on this project's own B8 cross-venue-coherence thesis (a MECE/logical-consistency arbitrage across venues, in-scope per the PLAYBOOK's edge thesis) — a materially stronger and larger-scale corroboration than anything logged for B8 to date. **Caveats, disclosed honestly:** this is a WebFetched abstract-level summary, not a full methodology read — "execution-aware" is the paper's own claim and has not been independently verified against THIS project's own fee/slippage model (`cost_model.py` + the new EXP-010 Polymarket schedule); the 10-venue panel is broader than this project's Polymarket+Kalshi-only scope, so the 2-4% average may not transfer 1:1 to the specific Polymarket/Kalshi pair; and near p=0.5 a 2-4% price gap could still be partly or fully consumed by Kalshi/Polymarket's own category-specific taker fees (Sports/Economics ≈5%, Crypto ≈7% per EXP-010's verified Polymarket schedule) if both legs must cross the spread. Logged as strong RECOMMENDATION-grade corroboration for prioritizing B8 step (ii), NOT a reproduced result on this project's own data — does not clear the high bar for a ROADMAP steer.
  - **Kalshi/CFTC fee-formula primary verification — attempted again per Run 24's filed next step (3), FAILED-CLOSED a second time (still secondary-sourced only, unchanged status).** Found and WebFetched a real June 24, 2026 CFTC filing (`cftc.gov/filings/orgrules/rules0624267243.pdf`) — but it governs **Kalshi's Perpetual Futures Contracts fee schedule** (a flat 0.07% of notional), a DIFFERENT product line from the binary event-contract `p×(1−p)` formula this project's EXP-010 needs verified; not applicable, correctly excluded rather than misapplied. The official first-party event-contract fee schedule (`kalshi.com/docs/kalshi-fee-schedule.pdf`, "Fee Schedule for July 2026") was located but is Vercel bot-gated (HTTP 429 / a security-checkpoint challenge page) on BOTH a direct WebFetch and a raw `curl` from this environment; Kalshi's own help-center article (`help.kalshi.com`) explicitly defers to that same gated PDF without stating the formula inline. Per FAIL-CLOSED discipline, the Kalshi-side fee formula stays secondary-sourced-only (3 consistent secondary sources, as logged 2026-07-23) — no number is promoted to primary-verified, and no EXP-010-Kalshi-analog work is triggered this run.
  - **Reconfirmation-only (no new numbers, not re-logged as evidence):** arXiv:2602.19520 "Decomposing Crowd Wisdom" (the Le 2026 calibration-decomposition paper, already the anchor source for EXP-002/003) and general "Kalshi vs Polymarket arbitrage" SEO/blog content (`clawarbs.com`, `dev.to`, GitHub bot repos) resurfaced with no new primary numbers — consistent with Runs 18/26/28's prior finding on this identical query shape.
  - **Regulatory/venue-risk item, logged as DATA (not an alpha, relevant to OA-2 venue-ToS/jurisdiction eligibility):** on 2026-07-14 the CFTC STAYED a Kalshi emergency rule that would have force-cancelled Michigan-resident trades under a state-court order (Michigan sought to compel cancellation + threatened $120k/day fines); the CFTC directed Kalshi to continue honoring all trades "as it would in the ordinary course of business," citing market-integrity risk from retroactive unwinding. A live, unresolved state-vs-federal jurisdiction dispute over Kalshi's regulatory status — worth the owner's awareness for OA-2, not a finding that changes any alpha track.
...
## 2026-07-27 — Research Run 31: EXP-009's founding citation does NOT survive primary-source verification, and a second, larger Kalshi employment-series diagnostic (KXU3, N=347/58 events, 2021-2026) reinforces the same small-reliability finding as Run 30's KXJOBLESS read. EXP-009 CLOSED (premise-not-supported). Plus an independent 30B-event academic corroboration for the standing C8 cost-realism constraint, and one new falsifiable proposal (EXP-012, spread/liquidity capture — filed, sequenced behind C8/B8). RECOMMEND-only, no code shipped, no ROADMAP steer.

**Orientation.** Read RESEARCH_MEMORY.md/RESEARCH_PLAYBOOK.md/ROADMAP.md/VISION.md/BUSINESS_CASE.md/GROWTH_STATUS.md first. `git log` HEAD is `14e6143` (2026-07-26 bookkeeping, #438, reconciling EXP-006b figures) — the prior alpha-relevant commit is #437 (2026-07-26, EXP-006b: fade-the-spike stays refuted, but the audit found a real leakage bug affecting every corpus this project has built + filed **ROADMAP C8** — backtests price at the CLOB midpoint and never pay a spread, a **5x-33x** understatement of real cost, now the binding *methodological* constraint "ahead of any new alpha." Binding constraint otherwise unchanged since 07-25: bucket-calibration (EXP-002/003/005/011) and fade-the-spike (EXP-006/006b) stay REFUTED; B8 is blocked on step (ii) (egress-gated historical co-listed corpus); EXP-009 was `proposed`, step 0 (KXJOBLESS) run 07-25 (tempering, not proven) with a pre-registered next step: the monthly unemployment-rate ladder (KXECONSTATU3/KXU3) with release-event clustering.

**Egress check (self-validated): open.** Direct `curl` to gamma-api/clob.polymarket.com, api.elections.kalshi.com, huggingface.co all HTTP 200 from this run's own environment.

**PART 1 — EXP-009 step 1: KXU3 (unemployment-rate monthly strike ladder) reliability decomposition, release-event-clustered. PRE-REGISTERED before fetching** (design fixed in a scratch file before any decision-price call; not committed, mirroring Run 30's precedent — this run's tool-call order is the timestamp).

- **Series identification (live-verified before fetching decision data).** `KXECONSTATU3` (the series name Run 28 originally found) turns out to be a RECENT rename — its own `/historical/markets` query returns only 5 monthly events (Dec-2025..Apr-2026), matching Run 28's live-tier finding almost exactly. `GET /series?category=Economics` surfaces the actual long-running series: `series_ticker=KXU3` reaches **58 monthly events, 537 resolved markets, Aug-2021 through Apr-2026** (spanning both the legacy `U3-*` and current `KXU3-*`/`KXECONSTATU3-*` ticker eras — verified the 5 KXECONSTATU3 events are byte-identical ticker strings to 5 of KXU3's 58, i.e. the SAME markets dual-registered, not double-counted). Decision: query `KXU3` alone.
- **Design (locked before fetching):** `historical=True` tier; `decision_lead=3 days` (UNCHANGED from Run 30's KXJOBLESS choice — comparability, not a fresh tuning DOF); `period_interval=60`; 1.5s inter-request pacing + 1 backoff retry (Run 30's fix for Kalshi's 429, carried over unmodified); event key = ticker's first two dash-segments (Kalshi's own event grouping, live-verified). Two pre-registered reads: (1) a pooled per-market Murphy decomposition (identical method to Run 30's KXJOBLESS read); (2) an event-clustered bootstrap on the Brier improvement over base rate (resampling the 58 EVENTS, not the 347 rows, with replacement — operationalizing the Run 22/EXP-006b correlated-strike-ladder caution directly, reusing the SAME cluster-bootstrap technique the EXP-006b audit used for its own correlation check).
- **RESULT (N=347 leakage-safe bucket-market records built from 537 discovered, 190 skipped — no qualifying pre-resolution tick, concentrated in the 2021-2022 tail; spans all 58 discovered events):** `base_rate(YES)=0.4467` (these are threshold/"greater-than" ladders, not single-winner buckets, so multiple contracts per event legitimately resolve YES — unlike KXJOBLESS's single clean market). **`crowd_brier=0.1184` vs `base_rate_brier=0.2472` — the crowd beats naive base-rate guessing by a WIDE margin** (nearly half the base-rate Brier). Murphy decomposition: **`reliability=0.0073`, 6.2% of total Brier** — SMALLER than KXJOBLESS's already-small 8% share; `resolution=0.1327` (large — the ladder structure gives the crowd strong discriminative power, expected); `uncertainty=0.2472` (dominant, as with every near-binary macro release). Approximation check: `reliability - resolution + uncertainty = 0.1219` vs `crowd_brier=0.1184` — a ~3% relative residual from 5-bin discretization (using per-bin means rather than each observation's own probability), the expected slop at this bin count, not a computation error. **Event-clustered 95% bootstrap CI on the Brier improvement (base_rate_brier − crowd_brier): [0.0952, 0.1531], excludes zero** — the crowd's outperformance of naive base-rate guessing is robust even accounting for within-event correlation; design effect only **1.31** (modest, in the same range as EXP-006b's 1.11-1.15), i.e. the strike-ladder correlation, while real, does not manufacture the result.
- **Per-bin table shows a DIFFERENT, honest, second-order pattern that must not be over-read:** every one of the 5 bins has `mean_pred > emp_freq` (gaps +0.038 to +0.163, largest in the sparse [0.4,0.6) bin, n=28). A uniform-direction gap across every bin is NOT what genuine crowd overconfidence usually looks like in this project's own prior reads (KXJOBLESS's per-bin gaps were MIXED-sign) — it is exactly the signature the already-disclosed `_candle_price` illiquid-one-sided-book ASK-fallback bias (inherited unchanged from #408/#409/Run 30) would produce: a systematic upward bias in the *recorded* crowd price, uniform in direction, on thin books — and these monthly-only-traded threshold-ladder buckets are plausibly thinner than KXJOBLESS's weekly clean single market. **This run did not instrument the fallback-trigger rate, so it CANNOT distinguish genuine mid-range miscalibration from this known measurement artifact — reported as an open, unresolved caveat, not a finding, per "prefer insufficient data over reading noise."** The reliability AGGREGATE stays small regardless (6.2%) because the two large, low-gap extreme bins (108+120=228/347=66% of the sample) dominate the weighted sum; the mid-range bins carrying the larger gaps are a minority of the mass.
- **Self-validation / fail-closed disclosure:** corpus NOT frozen (a re-run can differ); the ask-fallback fidelity caveat is inherited, not re-verified this run; 35.4% of discovered markets were skipped (no qualifying pre-resolution tick, worse for older markets) — a liquidity-selection bias in the SAME optimistic-upper-bound direction as every prior venue read; N=58 events clears the >=30-event diagnostic floor this run pre-registered but stays short of the project's standing >=100-event floor for any significance-gated backtest claim — this stays a **diagnostic**, never a validated/refuted backtest.

**PART 2 — the founding citation for EXP-009 does NOT survive primary-source verification (the load-bearing correction this run).** Every prior run citing NBER w34702 ("Kalshi and the Rise of Macro Markets," Diercks/Katz/Wright) for "employment markets show the weakest calibration" / "significant systematic mispricing" disclosed the SAME caveat: *"exact magnitude NOT verified — the primary PDF could not be text-extracted... logged directionally only, per FAIL-CLOSED."* This run the PDF (`nber.org/system/files/working_papers/w34702/w34702.pdf`) was successfully text-extracted (`pip install pypdf` — no repo code touched, a one-off research-environment tool, not committed) and read directly, for the first time. **It does not say what three prior runs' write-ups attributed to it.** Its own Table 3 (Kalshi vs. Bloomberg-consensus forecast accuracy) shows the unemployment-rate row with NO significance stars for any Kalshi statistic (mean/median/mode), and the paper's own sentence directly under the table reads: **"In no case is Kalshi significantly worse than the Bloomberg consensus."** Its probability-integral-transform calibration section concludes the unemployment/CPI/Fed-funds densities **"appear to be fairly well calibrated,"** with only "borderline" evidence that risk premia mildly overstate high-inflation/high-unemployment tail probabilities — not the "significant systematic mispricing" framing. **The word "Brier" does not appear anywhere in this paper — zero occurrences**, confirmed by direct text search of the full 41-page extraction. A same-session WebSearch (run BEFORE this correction was known) resurfaced a search-engine-synthesized answer quoting "Brier score of 0.1302 for unemployment... 0.0001 for Federal Funds Rate" and attributed it to two DIFFERENT-titled ResearchGate listings ("Information Efficiency Across Macroeconomic Prediction Markets," "Market Efficiency and the Favorite-Longshot Bias in Unemployment Prediction Markets") both describing the identical "2,668 settled contracts, July 2021-June 2026" dataset as w34702 — ResearchGate 403-blocked every fetch attempt (bot-gated, same class of failure as the Kalshi fee PDF below), so **these specific numbers and even the independent existence of those two listings as distinct real papers could NOT be verified and are NOT reported as findings** — FAIL-CLOSED, per the same discipline applied to every other bot-gated source this project has hit. **This is a genuine research-integrity finding, not a data result:** a claim was carried across three research runs (07-22, 07-23/25 mentions, motivating EXP-009's entire premise) on the strength of a search-engine paraphrase that, now checked against the actual primary text, is not supported by it — exactly the failure mode "self-validation: declare the sources your analysis depends on; FAIL CLOSED if a source cannot be reached" exists to catch, except here the source COULD be reached this run and, once read, contradicted the paraphrase rather than merely being unreachable. Recorded so a future run does not re-cite w34702 for this claim.

**Combined verdict — EXP-009 CLOSED, premise-not-supported (not a backtest refutation — no predictor was ever built or traded).** Two independent lines of evidence now agree: (1) the paper EXP-009 was founded on does not, on direct reading, claim what it was cited as claiming for Kalshi employment/unemployment markets; (2) two real, leakage-safe, pre-registered Kalshi employment-series diagnostics — KXJOBLESS weekly (Run 30, N=67, reliability 8% of Brier) and KXU3 monthly (this run, N=347/58 events, reliability 6.2% of Brier) — both find a SMALL reliability term, i.e. little headroom for a calibration-bucket predictor on either the venue's weekly or monthly employment product. Combined with EXP-002/003/005/011 (Polymarket, 4 corpora, all refuted) and this pair, the calibration-bucket-search PROGRAM as a whole has now been run against Polymarket (4 categories/corpora) and Kalshi (2 employment cadences) with no reliability-term headroom found anywhere. This is a portfolio-level finding: the calibration-bucket MECHANISM, not just one series, looks exhausted as a source of edge with the venues/categories tested to date.

**PART 3 — external sweep, one genuine corroboration for the ALREADY-FILED C8 constraint, one FAIL-CLOSED, one reconfirmation, one DATA item.**
- **Genuine, primary-verified corroboration for ROADMAP C8 (the standing "ranks ahead of any new alpha" constraint filed 07-26):** WebFetched arXiv:2604.24366v2, "The Anatomy of a Decentralized Prediction Market: Microstructure Evidence from the Polymarket Order Book" — 30 BILLION order-book WebSocket events over 52 days (2026-02-21 to 2026-04-15), joined to the on-chain trade record. Directly quotes: median quoted half-spread **~200bps in the central [0.4,0.6] price range**, climbing to **1,300-1,800bps for markets below 0.10** (decile-0 median 1,818bps) — independently corroborating, from a dataset three orders of magnitude larger, the SAME direction and a similar magnitude to EXP-006b's own live-measured real half-spreads (2.5%/5.8%/16.7% by band) that motivated C8. This is a real, quotable, primary-verified academic anchor the factory can cite when it builds the spread-aware cost model — logged here (RESEARCH_MEMORY is read first each run) rather than as a ROADMAP edit, since C8 is already filed and this is corroboration, not a new steer.
- **FAIL-CLOSED (a 3rd consecutive run):** `kalshi.com/docs/kalshi-fee-schedule.pdf` retried per Run 30's filed next step — still 429/bot-gated on WebFetch. A WebSearch snippet describes a "0.07 coefficient... fee = round(multiplier x price x (1-price), 2)" formula consistent with the 3-secondary-source figure already on file, but this is again a search paraphrase of an unreadable document, not a primary read — stays secondary-sourced-only, no number promoted.
- **FAIL-CLOSED, unverified pointer (not logged as evidence):** a WebSearch surfaced an SSRN working paper (Krause, June 2026) claiming persistent arbitrage (~4.87% mean profit after fees, legislative-bill and Fed-meeting event studies) — `papers.ssrn.com` 403-blocked on WebFetch, so the magnitude is UNVERIFIED search-engine synthesis of an unreadable source, not confirmed. Even if the numbers hold up, single-event case studies (one bill, one FOMC meeting) carry the same N=1-2 generalizability risk this project has flagged before (Run 22's standing caution) — not pursued further this run.
- **Reconfirmation only (A9, Robinhood Predict):** WebSearched + WebFetched Robinhood's own "Agentic Trading overview" support surface — the MCP-based agentic trading beta (launched 2026-05-27) is confirmed STILL stock-only; event contracts/prediction markets are explicitly named "on the roadmap for the second half of 2026," not shipped. A9's gate 1 (availability) stays unmet — same conclusion as Run 29 (07-23), no new evidence either direction, no ROADMAP change.
- **DATA item (not an alpha, OA-2 relevance, escalation of the item Run 30 logged):** the Kalshi/Michigan state-vs-federal jurisdiction dispute (Run 30 logged the 2026-07-14 CFTC stay) has escalated: the CFTC has since filed suit against Michigan and is reported pursuing the same federal-preemption argument against Arizona, Connecticut, Illinois, Kentucky, Minnesota, New Mexico, New York, Rhode Island, and Wisconsin; the CFTC chair is quoted calling the fight "existential" and open to litigating to the Supreme Court. Relevant to OA-2 (venue ToS/jurisdiction eligibility) if/when the owner considers going live — not an alpha finding, no PENDING_OPS edit (informational only, per Run 30's own precedent of logging this class of item in RESEARCH_MEMORY, not editing PENDING_OPS.md).
- **Market-structure context (not an alpha):** Kalshi reports institutional trading volume +800% over six months and >90% US prediction-market share (~$22B May-2026 funding round); consistent with, and reinforcing, this project's own standing "43% spread compression" / increasing-efficiency caution already on file for EXP-001 — the crowd this project is trying to beat is plausibly getting SHARPER over time, not softer, which raises rather than lowers the bar for any future calibration-family alpha.

**NEW FALSIFIABLE PROPOSAL — EXP-012, passive spread/liquidity capture (filed to `experiments[]`, explicitly sequenced BEHIND C8 and B8, not urged as the immediate next step).** The playbook names "spread/liquidity capture" as an in-scope edge source; no experiment in this project's history has ever targeted it — every backtest to date assumes TAKER execution (crossing the spread). PART 3's arXiv:2604.24366v2 finding (200bps center vs 1,300-1,800bps at price extremes) plus Polymarket's and Gemini's own live maker-rebate programs (found via WebSearch this run, promotional/secondary, not independently verified) make the shape of a possible edge concrete: post resting limit orders and earn the spread + rebate instead of paying it. Hypothesis, N, OOS plan, costs, and how-it-could-be-wrong are written into `experiments[]` below. **Sequencing, stated plainly:** this needs NEW backtest infrastructure this project does not have (order-book replay + a fill-probability/adverse-selection model — the existing `walk_forward`/`cost_model` assume a single taker fill at a modeled price, not a resting order's uncertain fill), and adverse selection (a resting order fills disproportionately right before the market moves against it) is the standing reason wide spreads exist in the first place, not obviously free money. Filed as a backlog item for when C8 (cost-model realism, ranked first) and B8 step (ii) (the standing highest-EV track) allow bandwidth — building this before C8 lands would compound one already-known-flawed cost model with a second, harder one.

**Verdict:** edge-not-proven; no revenue field moved; `business_case_strength` stays **B**. This run's real contribution is a research-integrity correction (a load-bearing citation did not hold up once actually read) plus closing out a genuinely-tested premise across two Kalshi employment cadences, both converging — an honest close counts as work done, not a null result to apologize for.
**NEXT (pre-registered, do NOT p-hack):** (1) EXP-009 is DONE, premise-not-supported — do not re-open the Kalshi-employment calibration-bucket line without a NEW primary-verified claim (not a search paraphrase) naming a specific, different mechanism. (2) B8 step (ii) remains the standing highest-priority track (egress-gated historical co-listed BTC/ETH corpus + common-instant snapshot) — unchanged, this run adds no new evidence either direction on it. (3) C8 (cost-model realism) is FACTORY-owned and stays ranked ahead of any new alpha per the 07-26 audit; this run's arXiv:2604.24366v2 finding is a ready-to-cite external anchor for that build. (4) EXP-012 (spread/liquidity capture) is filed as backlog, explicitly behind (2) and (3). (5) Retry the Kalshi fee-schedule PDF from a non-bot-gated path if ever prioritized (low urgency, 3rd consecutive fail, B8/C8 are the standing higher-EV tracks).
Binding constraint UNCHANGED: `business_case_strength` **B**, no validated real-money OOS edge on any tested mechanism. Real-money brake untouched (no order, no gate flip, no cap change, no capital, no live-trading surface touched). `engine_pct` 74 unchanged.

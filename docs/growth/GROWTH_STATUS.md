...
GROWTH_STATUS:
  ...
  experiments:
    ...
    - id: EXP-011
      name: "Settlement-Discount Term-Structure Diagnostic (capital lock-up / carry mispricing)"
      status: proposed
      proposed_date: 2026-07-24
      edge_source: "capital-cost/carry mispricing on public settlement-timing data (in-scope per PLAYBOOK — 'faster/cheaper execution,' 'spread/liquidity capture'; NOT crowd-belief calibration, a genuinely different mechanism than every prior tested family)"
      hypothesis: >
        arXiv:2605.31431 ("When Certainty Is Not Worth It: Capital Lock-Up and Settlement
        Discounting in Prediction Markets," verified 2026-07-24 via direct WebFetch of the
        arxiv abs page, full verbatim abstract) shows collateralized-market near-certain
        contracts (price > 0.95 or < 0.05) embed a positive, maturity-dependent
        "annualized settlement wedge" (ASW) reflecting the time value of capital locked
        until oracle settlement — adjusting for it explains roughly 48-88% of the raw
        near-certainty horizon gradient (longer-dated near-certain contracts trading
        cheaper than shorter-dated ones at the same implied probability), leaving a
        12-52% residual NOT explained by rational discounting alone. HYPOTHESIS
        (falsifiable, measurement-first — no strategy/PnL claim yet): computing the same
        ASW decomposition on this project's OWN already-fetchable leakage-safe
        `HistoricalMarket` corpora (Polymarket Gamma + Kalshi `/historical/*`, both
        already built, #408) reveals a NON-trivial, maturity-bucketed residual wedge
        (after netting a pre-registered capital-cost benchmark) that is NOT explained by
        negRisk-eligibility alone — a candidate structural mispricing distinct from crowd
        belief, and a candidate explanation for why EXP-002/003/005's refuted
        bucket-calibration signal concentrated in the extreme-confidence (near-0/1)
        buckets every one of their F10 reports flagged (those strategies treated a
        rationally-discounted near-certain price as "underpriced" crowd belief).
      min_sample_n: >
        >= 200 resolved binary markets with price > 0.95 or < 0.05 at decision time AND a
        computable decision-to-resolution gap, spanning >= 3 maturity buckets (e.g. <7d,
        7-30d, >30d) with >= 30 per bucket for any bucket-level claim (mirrors the
        standing EXP-002 per-bucket floor). Reuses corpora already partially collected
        (EXP-002/003/005's committed Polymarket pulls, EXP-009's Kalshi `/historical/*`
        pulls) — may need a fresh combined pull if the near-certain subset of any single
        existing corpus is too thin.
      oos_plan: >
        THIS IS A DIAGNOSTIC, NOT A BACKTEST — no walk-forward split needed for step 1.
        (1) Filter existing/fresh leakage-safe records to near-certain resolved markets;
        (2) PRE-REGISTER a capital-cost benchmark (e.g. a fixed short-term risk-free rate
        proxy, or a simple constant assumption disclosed up front) BEFORE computing any
        wedge — do not choose the benchmark after seeing results; (3) compute each
        market's implied ASW and report the mean/CI per maturity bucket and per
        negRisk-eligible vs. not; (4) test whether the CROSS-MATURITY spread (long-bucket
        ASW minus short-bucket ASW) is statistically distinguishable from the benchmark
        rate (bootstrap CI). Only if step (4) shows a real, non-fragile residual would a
        SEPARATE, freshly-pre-registered relative-value backtest (favor buying near-
        certain contracts in the maturity/collateral-type bucket with the largest
        residual wedge; same B2/F10/F11 gates as every other alpha) be proposed as a
        follow-on — not attempted in this step.
      cost_assumptions: >
        Step 1 is a pure price/timestamp diagnostic — no cost_model.py involvement (no
        trade is simulated). A follow-on backtest, if step 1 warrants one, would reuse
        `cost_model.py` unmodified (2% fee + 0.5% slippage) exactly like every prior
        alpha test.
      significance_threshold: >
        Bootstrap CI on the cross-maturity ASW spread must exclude the pre-registered
        capital-cost benchmark rate to count as a "residual exists" finding; Bonferroni
        if testing >1 maturity-bucket pair. A near-zero or CI-spanning result is an
        honest "no exploitable residual" null, not a failure to report.
      how_it_could_be_wrong:
        - "The paper's ASW recovery methodology is described only in the abstract (not the full text) — this project's first-pass replication will necessarily APPROXIMATE rather than exactly reproduce their estimator; results are a candidate reproduction, not a validated replication, until the full method is read."
        - "The 12-52% 'unexplained residual' is the paper's OWN framing of what settlement mechanics do NOT explain — it is not asserted by the paper to be a tradeable edge; the residual could itself be driven by ordinary forecast uncertainty (a near-certain-but-not-certain contract genuinely has SOME chance of flipping) rather than a further pricing inefficiency. 'No residual edge, it's just remaining real uncertainty' is a live, honest alternative outcome."
        - "This project's `HistoricalMarket.resolution_time` may represent market CLOSE time, not the oracle's actual settlement/payout time (the quantity the paper's ASW is defined on) — these can differ by a real lag; this must be verified against the fetcher's actual field semantics before computing anything, not assumed equivalent."
        - "A cross-maturity 'relative value' framing is only near-riskless if both maturities carry genuinely negligible remaining forecast uncertainty (both truly near 0/1) — if the longer-dated 'near-certain' market still carries non-trivial flip risk, the measured wedge conflates real forecast risk with financing cost, and the diagnostic must disclose this conflation risk explicitly in its output, not silently net it out."
        - "The capital-cost benchmark choice (e.g. which risk-free proxy) is a free parameter — picking it AFTER seeing which choice makes a residual 'significant' would be p-hacking; it must be pre-registered before any computation, per the standing PLAYBOOK/VISION discipline this project has followed for every prior experiment."
      blocking_dependency: >
        None for step 1 — fully buildable from ALREADY-COMMITTED or freshly-fetchable
        leakage-safe corpora via existing, unmodified fetchers (Polymarket Gamma +
        Kalshi `/historical/*`, #408). The only prerequisite is a small, well-scoped new
        analysis script (a diagnostic, not a strategy) — factory-build scope, no new
        venue, no new credential, no egress dependency beyond what already works.
      factory_next_action: >
        Build a small, pure diagnostic (e.g. `settlement_discount_diagnostic.py`) that:
        (1) filters existing/committed `HistoricalMarket` records to near-certain
        resolved markets with a valid decision-to-resolution gap; (2) implements the
        pre-registered ASW computation; (3) reports the maturity-bucketed term structure
        + negRisk split, purely descriptively (no PnL, no gated strategy claim) — the
        same non-p-hackable class as B9's `per_category_diagnostics`. Only escalate to a
        real backtest if the cross-maturity residual survives its own pre-registered
        significance test. RECOMMEND-only this run — no code shipped, no ROADMAP steer.
  learnings:
    - "Research Run 30 (2026-07-24): a genuinely new, structurally-different candidate
      mechanism surfaced via direct WebFetch of arXiv:2605.31431 (\"Capital Lock-Up and
      Settlement Discounting in Prediction Markets\") -- near-certain contracts embed a
      rational, maturity-dependent settlement-discount wedge (48-88% of the raw
      near-certainty horizon gradient is explained by capital lock-up, not belief), with
      a 12-52% unexplained residual. This is a candidate STRUCTURAL explanation for why
      EXP-002/003/005's refuted bucket-calibration signal concentrated in the extreme-
      confidence buckets every one of their F10 reports flagged -- a rationally-
      discounted near-certain price can look like free miscalibration to a naive
      empirical-resolution-rate model. Filed EXP-011: a measurement-first diagnostic (no
      PnL, no new strategy) reusing existing leakage-safe Polymarket+Kalshi corpora,
      explicitly predicting the residual MAY be near-zero (an honest null is a valid
      outcome, not a foregone edge). SEPARATE finding: re-querying WebSearch for NBER
      w34702's exact employment-calibration Brier/bias numbers (the gap Run 28 left
      open) produced INTERNALLY INCONSISTENT numbers across two queries (0.1302 vs 0.1270
      Brier; -0.0695 vs -0.0891 bias) -- neither adopted; generalizes Run 28's
      fail-closed stance into a standing caution that WebSearch's summarized 'quotes' of
      academic statistics can drift between queries and must be independently re-verified
      via direct WebFetch before being cited as evidence. Also: Robinhood's OWN
      prediction-markets product (distinct from the stocks-only 'Robinhood MCP' agentic
      tool) is confirmed live and large (9B contracts, 1M+ users) but has NO documented
      public market-data API of any kind -- sharpens, does not change, A9's double gate
      (a factual note for A9's next edit, not a ROADMAP steer). RECOMMEND-only, no
      ROADMAP steer. Binding constraint (no validated real-money OOS edge on any tested
      mechanism to date) STANDS. Full detail: RESEARCH_MEMORY 2026-07-24 (Research Run
      30)."

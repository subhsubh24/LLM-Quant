...
GROWTH_STATUS:
  ...
  experiments:
    ...
    - id: EXP-009
      ...
      status: premise-not-supported
      proposed_date: 2026-07-22
      tested_date: 2026-07-27
      result: >
        CLOSED (Research Run 31, 2026-07-27) — premise-not-supported, NOT a backtest
        refutation (no predictor was ever built or traded; `CalibrationBucketStrategy` was
        never run against Kalshi employment data). Two convergent lines of evidence: (1) the
        founding citation (NBER w34702) was FINALLY successfully text-extracted this run and
        read directly for the first time — it does NOT say what three prior runs' write-ups
        attributed to it. Its own Table 3 shows the unemployment-rate row with NO
        significance stars for any Kalshi statistic and states "In no case is Kalshi
        significantly worse than the Bloomberg consensus"; its PIT-calibration section
        concludes the densities "appear to be fairly well calibrated." The word "Brier" does
        not appear anywhere in the paper (0 occurrences, full-text search). (2) Two real,
        leakage-safe, pre-registered Kalshi employment-series reliability decompositions now
        exist and agree: KXJOBLESS weekly (Run 30, N=67, reliability = 8% of Brier) and KXU3
        monthly strike-ladder (Run 31, N=347 records / 58 release-events, event-clustered,
        reliability = 6.2% of Brier, event-clustered 95% CI on the Brier-vs-base-rate
        improvement [0.0952,0.1531] excludes zero — the crowd beats naive guessing, design
        effect only 1.31). Both show SMALL reliability (miscalibration) terms — little
        headroom for a calibration-bucket predictor on either Kalshi employment cadence.
        Combined with EXP-002/003/005/011 (Polymarket, 4 corpora, refuted), the
        calibration-bucket-search PROGRAM has now been run against every (venue,
        employment-cadence) combination tested to date with no reliability headroom found
        anywhere — a portfolio-level finding, not just a single-series null. Full detail:
        RESEARCH_MEMORY 2026-07-27 (Research Run 31).
    ...
    - id: EXP-012
      name: "Passive Spread/Liquidity Capture (maker-side, not taker-side execution)"
      status: proposed
      proposed_date: 2026-07-27
      edge_source: "spread/liquidity capture (in-scope per PLAYBOOK edge thesis — the ONE named in-scope edge source this project has never yet targeted; every prior/current strategy assumes TAKER execution, crossing the spread)"
      hypothesis: >
        Posting resting limit orders inside the observed bid-ask spread (instead of crossing
        it as every existing strategy does) earns the realized spread — plus any venue maker
        rebate — net of adverse-selection losses and fees, producing positive expected PnL,
        PARTICULARLY in illiquid price bands where the spread is widest. Research Run 31
        WebFetched arXiv:2604.24366v2 (30B Polymarket order-book WebSocket events, 52 days,
        primary-verified) measuring a median quoted half-spread of ~200bps in the central
        [0.4,0.6] price range, climbing to 1,300-1,800bps below 0.10 — an order of magnitude
        wider than Polymarket's ~2-7% per-category taker fee (EXP-010) at the low-price
        extreme. Polymarket and Gemini both run live maker-rebate programs (WebSearched this
        run, promotional/secondary sources, NOT independently verified) that would add to any
        captured spread. This is a DIFFERENT edge mechanism than every strategy tested to date
        (all of which price-forecast and then cross the spread) — a null on the calibration-
        bucket or fade-the-spike families does not predict a null here.
      min_sample_n: >
        >= 100 independent resting-order episodes across >= 3 markets/categories with REAL
        fill outcomes (filled vs. not filled, and at what realized adverse-selection cost) —
        raw quote-side spread width alone (as measured by arXiv:2604.24366v2) is NOT
        sufficient; a captured-spread claim requires knowing whether and when the resting
        order would actually have filled, which needs order-book-level replay, not just
        settlement-price data.
      oos_plan: >
        NOT buildable against existing repo infrastructure — `walk_forward.py`/`cost_model.py`
        model a single TAKER fill at a computed price; there is no fill-probability or
        resting-order model anywhere in this codebase. A real test needs (1) a captured
        order-book time series (Polymarket CLOB WS feed — this project's own
        `websocket_feeds.py` already ingests the live top-of-book, but does not archive a
        historical order-book corpus) or a documented public source of historical L2 depth,
        and (2) a fill-simulation model (does a resting order at price p get filled before the
        market moves away, and if filled, what is the REALIZED adverse-selection cost vs. the
        naive "captured the quoted spread" assumption). Chronological split once (1)+(2) exist,
        same B2/F10/F11 gate discipline as every other experiment in this file.
      cost_assumptions: >
        The main "cost" here is NOT a fee (makers are often free/rebated) but ADVERSE
        SELECTION — a resting order fills disproportionately right before an unfavorable
        price move (the standing economic reason wide spreads exist in the first place, not
        free money sitting on the table). No existing cost_model.py primitive models this;
        it would need to be built new, and naively assuming "spread captured minus zero" would
        be exactly the kind of unrealistic-cost mistake ROADMAP C8 was just filed to correct
        on the taker side — do not repeat that mistake on the maker side.
      significance_threshold: "Same B2/F10/F11 discipline as every other experiment (95% CI excluding 0 on paired bootstrap PnL improvement over a taker-execution baseline; F10 non-fragile); Bonferroni if tested across multiple price bands."
      how_it_could_be_wrong:
        - "Adverse selection could easily exceed the captured spread, especially in fast-moving political/news markets — wide spreads are priced BY market makers precisely to compensate for this risk, not an inefficiency waiting to be captured for free."
        - "Maker rebate programs are recent/promotional (found via WebSearch, not independently verified) and may not persist, may be crypto-market-specific (Polymarket's own program is scoped to 15-minute crypto markets per the source found), or may not generalize to the political/economic markets this project otherwise focuses on."
        - "Capital gets tied up in resting orders with uncertain fill timing — an opportunity-cost / capacity dimension none of this project's existing capacity work (which assumes instant taker fills) currently models."
        - "This needs materially NEW backtest infrastructure (order-book replay + fill simulation) — a bigger lift than any experiment this project has built to date, and building it before ROADMAP C8 (cost-model realism, filed 2026-07-26, ranked 'ahead of any new alpha') lands would compound one already-known-flawed cost model with a second, harder one."
        - "arXiv:2604.24366v2's spread measurements are QUOTED spreads from the public WS feed, not a measurement of what a maker would actually realize after adverse selection — the paper itself does not attempt to distinguish quoted from executable/realized spread capture (Research Run 31's own WebFetch confirmed this gap directly)."
      blocking_dependency: >
        Sequenced explicitly BEHIND ROADMAP C8 (cost-model realism, factory-owned, ranked
        first) and B8 (cross-venue coherence, the standing highest-EV alpha track). Needs new
        order-book-archival + fill-probability-model infrastructure this project does not have
        — NOT buildable as a small, well-scoped increment the way EXP-010 was. Filed to the
        backlog per the DECISION COROLLARY (no speculative skeleton before the infrastructure
        gap is acknowledged and sequenced).
      factory_next_action: >
        NONE this run — backlog item, not the next buildable step. When C8 and B8 step (ii)
        allow bandwidth: scope a minimal order-book-archival + fill-simulation MVP (e.g.
        replay the existing `websocket_feeds.py` top-of-book stream going forward, rather than
        assuming historical L2 depth is retrospectively available — Kalshi/Polymarket do not
        retain historical book depth for settled markets, per the EXP-006b capacity-measurement
        finding already on file) before writing any strategy code.
  learnings:

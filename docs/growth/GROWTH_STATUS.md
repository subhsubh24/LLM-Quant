...
GROWTH_STATUS:
  ...
  next_actions:
    ...
    - "URGENT + loop-buildable (no owner action, no data needed): fix or remove the fabricated `KNOWN_WHALES` seed in `whale_feed.py` (a hardcoded address does not match the real 'Theo4' wallet; a third seed is a self-evidently fake sequential-hex placeholder) — either source addresses verifiably from Polymarket's own public API/leaderboard at build time, or drop the hardcoded seed and rely solely on dynamic `/holders` discovery. Then gate `WhaleCopyTradingStrategy` and `WeatherArbitrageStrategy` (both currently wired unconditionally into `orchestrator._build_default_scanner()` with zero tests and zero ROADMAP/B3 tracking) behind a B3 `PROPOSED` registry entry + a `strategy_audit.py` forensic pass BEFORE they run in the default scanner, and add a ROADMAP line item for each so 'evidence-based done' applies retroactively. See RESEARCH_MEMORY 2026-07-01 for the full finding."
  owner_blockers:

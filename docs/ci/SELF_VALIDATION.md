...
SELF_VALIDATION:
  ...
  capabilities:
    ...
    - id: half_spread_cost_realism
      ...
      validates_via: "test_half_spread.py (REGISTERED in the blocking gate), 50 tests: the DEFAULT (no half_spread_model) path is pinned bit-identical against the recomputed pre-C8 formula and DEFAULT_COST_MODEL equality/hash is unchanged; the committed band table is REGENERATED from data/depth_probe_polymarket.json and must match exactly, so a hand-typed table cannot drift from the bytes it claims to summarize; an empty band RAISES rather than defaulting; gapped/short band sets are rejected at construction; the 747-book bands carry n=None (that source published no per-band n) while depth-probe bands carry a real n, so no sample size is invented; the measured cost is asserted costlier than the flat rate below 0.90 AND CHEAPER on the exit leg above 0.98 (a real inversion an auditor found the original blanket claim hid — the near-certain book is genuinely tight; worth +$1.29 against a -$887.89 correction, immaterial but asserted rather than hidden); sell proceeds cannot go negative in the sub-cent band; and the half_spread_model ENTERS _seed_hash — two runs differing only in the model must not collide, proven non-tautologically by also asserting the PnL differs (both fingerprint tests FAIL on pre-change code with a real same-hash/different-PnL collision, 13129.34 vs 12975.41)."

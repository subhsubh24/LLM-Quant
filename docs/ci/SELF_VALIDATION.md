...
SELF_VALIDATION:
  ...
  capabilities:
    ...
    - id: exp010_cost_realism_rescore
      ...
      status: validated
    - id: half_spread_cost_realism
      desc: "ROADMAP C8 measured bid/ask half-spread cost model. Polymarket's CLOB /prices-history serves the book MIDPOINT, not a traded price, so every backtest in this repo entered and exited at an unattainable price and paid only the flat DEFAULT_SLIPPAGE_RATE=0.005 to cross. `half_spread.py` supplies a price-banded MEASURED half-spread from two independent live measurements (747 political books recorded in EXP006B_RESULT.md; 62 two-sided books committed at data/depth_probe_polymarket.json), `CostModel.half_spread_model` REPLACES the flat slippage with it when set, and `scripts/rescore_with_spread.py` re-scores both refuted families under flat + all three measured models. Result: the project's ONLY F11 significant_positive cell (EXP-006b fade th=0.15, N=141, +$2,441.03, CI [+364.93, +4736.05]) falls to indistinguishable_from_zero under EVERY measured model (least punitive: +$1,735.96, CI [-209.03, +3842.10]). The harness ALSO reports a theoretical half-tick FLOOR as a labelled bound (not a model, excluded from the survivor verdict) under which the cell KEEPS significance (+$2,068.26, CI [+53.80, +4296.84]) — so the span has both ends and the ~30% margin is visible rather than implied. The cell was already is_validated_edge=False under FLAT cost (F10-fragile), so C8 removes its last encouraging property, it does not change the pre-registered verdict."
      validates_via: "test_half_spread.py + test_rescore_with_spread.py (BOTH REGISTERED in the blocking gate): the DEFAULT (no half_spread_model) path is pinned bit-identical against the recomputed pre-C8 formula and DEFAULT_COST_MODEL equality/hash is unchanged; the committed band table is REGENERATED from data/depth_probe_polymarket.json and must match exactly, so a hand-typed table cannot drift from the bytes it claims to summarize; an empty band RAISES rather than defaulting; gapped/short band sets are rejected at construction; the 747-book bands carry n=None (that source published no per-band n) while depth-probe bands carry a real n, so no sample size is invented; the measured cost is asserted costlier than the flat rate below 0.90 AND CHEAPER on the exit leg above 0.98 (a real inversion an auditor found the original blanket claim hid — the near-certain book is genuinely tight; the harness EMITS the figure — 11 exit legs at or above 0.90 gaining +$0.63 net, of which the 2 legs at or above 0.98 gain +$1.28 alone; immaterial, and asserted rather than hidden); sell proceeds cannot go negative in the sub-cent band; and the half_spread_model ENTERS _seed_hash — two runs differing only in the model must not collide, proven non-tautologically by also asserting the PnL differs (both fingerprint tests FAIL on pre-change code with a real same-hash/different-PnL collision, 13129.34 vs 12975.41)."
      mode: committed_artifact         # pure offline re-score of committed corpora via the walk-forward + fade engines — no network, no credentials
      requires_env: []                 # NONE — reuses data/depth_probe_polymarket.json + the two committed corpora
      active: true
      ci_validatable: true             # fully validated in-gate with no secret — committed artifact + seeded engines reproduce identically
      real_flow_note: "This capability carries NO edge claim and reaches NO revenue field — it makes an already-collected result MORE honest by charging a measured cost the data never charged. Both half-spread measurements come from OPEN, volume-ordered (LIQUID) markets, so they UNDER-state a random market's spread and every model is a LOWER bound on real crossing cost: a cell that dies under a lower bound is dead, while a cell that survived would be a CANDIDATE requiring >=3 FRESH adversarial Opus auditors, never an edge. The harness reports flat, 747book, depth_probe and conservative side by side precisely so a verdict cannot rest on the single most convenient cost number. The counterintuitive BUCKET movement (a costlier entry makes the loss SMALLER, -$3,228.02 -> -$3,192.62 on an IDENTICAL 39-trade set) is not a bug and is not hidden: the harness emits budget_deployed_usd and contracts_held so the mechanism is visible — a costlier entry shrinks the cost-net edge, which shrinks Kelly, which shrinks the position, the same effect ROADMAP E8 already records for the per-category caps."
      status: validated
    - id: forward_depth_capture
      desc: "ROADMAP E11 forward depth capture. Retrospective depth is UNOBTAINABLE from Polymarket (Gamma serves liquidity:null on resolved markets; CLOB /book 404s on a settled token, both verified live), so no PAST corpus can ever be capacity-tested and no committed corpus can have its entry price OBSERVED rather than estimated — the `liquidity: null` on every frozen OOS record is a venue data-availability FACT. Depth IS observable going forward, so the orchestrator now captures the pre-trade order book at the decision instant (after every gate, before OrderRequest) onto a nullable `book_json` column on PredictionAuditLog, bounded to 40 ask levels to match capacity_probe.LADDER_LEVELS. OBSERVATION ONLY: nothing in the trading path reads it back. Public unauthenticated CLOB endpoint; NO credential. On by default (CAPTURE_ORDER_BOOK_AT_DECISION=0 disables) because past depth is unrecoverable, so a default-off capture accrues nothing and the measured capacity curve stays permanently unbuildable."
      validates_via: "test_audit_log.py (REGISTERED in the blocking gate), 32 tests: a captured book round-trips through the store; the ladder is BOUNDED and the bound is compared against capacity_probe.LADDER_LEVELS by importing BOTH constants (not pinning a literal in one file that would not notice the other moving); band depth scalars are proven computed over the FULL ladder via a 120-level book whose 5c band spans 51 levels past the 40-level cut; the stored ladder is proven BEST-FIRST by feeding a worst-first book (the real wire shape #434 fixed); every failure path (raise / timeout / None / one-sided / malformed) records None and the scan CONTINUES, never a fabricated quote; capture is proven NOT to alter the decision, size or order (identical scan cycles with capture on vs off, field-by-field); and the degraded write path is exercised against a database whose table LACKS book_json. Mutation-checked: removing the sort, or moving band depth onto the truncated ladder, each FAIL."
      mode: in_process_deterministic   # deterministic offline tests; one live read-only smoke run, disclosed below as non-CI
      requires_env: []                 # NONE — public CLOB order book, no credential; CAPTURE_ORDER_BOOK_AT_DECISION is a non-credential toggle
      active: true
      ci_validatable: true             # fully validated in-gate with no secret
      real_flow_note: "This capability carries NO edge claim and reaches NO revenue field — it RECORDS data, it does not price, size, gate or order anything. It is not a stub hiding an un-exercised critical path: the decision path it attaches to is the real paper scan/execute path the runtime harness drives, and the capture's non-interference is asserted against that real path rather than a mock. Live read-only smoke (non-CI, disclosed) captured real ladders from Polymarket and honestly recorded two genuinely one-sided books as no-snapshot. KNOWN DEPLOYMENT CAVEAT: `book_json` is mapped unconditionally, and this repo's only schema path (create_all) never ALTERs an existing table — so on the deployed database the insert is rejected. The writer DEGRADES (a raw parameterized retry naming the pre-E11 columns) so the ROADMAP G3 audit trail survives intact and only the new capture is lost, with a single ERROR naming the one-line fix; enabling capture there is PENDING_OPS OA-20, an owner action. STATED LIMITS: only the ASK ladder plus the bid touch is stored, so a SELL-side capacity question cannot be answered from these rows; and depth is captured on the executed-order path only, not on rejected decisions."
      status: validated
    - id: capacity_impact_analysis
      desc: "Capacity / market-impact analysis (QUALITY_SCORECARD backtest_integrity residual (b)). Two parts: (1) `capacity.py` — a pure, deterministic capacity curve + floor-feasibility solver over the SHARED cost model, answering how large a per-trade stake can be before own-order impact eats the edge; (2) `capacity.walk_book()` / `implied_impact_coeff()` — MEASURED impact obtained by walking a real captured ask ladder, with NO free parameter, which is how DEFAULT_IMPACT_COEFF stops being an unchecked placeholder. Established that retrospective depth is UNOBTAINABLE from Polymarket (resolved markets serve liquidity:null and CLOB /book 404s), so the `liquidity: null` on every committed OOS record is a venue data-availability fact, NOT a fetcher threading bug."
      validates_via: "test_capacity.py (REGISTERED in the blocking gate), 28 tests: impact drag is never negative and is monotone non-decreasing in size; a deeper book is never more expensive; the model converges to the flat cost as depth grows (so a capacity analysis never contradicts the flat-cost backtest in the limit); NON-POSITIVE DEPTH RAISES rather than reporting infinite capacity on an empty book (the most dangerous possible output); a non-positive edge is REFUSED rather than sized (the project's actual state — capacity is not then the binding constraint); the capacity ceiling is real (past the peak stake, more capital earns LESS); walk_book averages only levels it actually consumes and reports a PARTIAL FILL rather than extrapolating the unfilled tail; and implied_impact_coeff REFUSES a partial fill, because a partial fill understates true cost and would imply a coefficient that is too LOW (wrong in the dangerous direction)."
      mode: committed_artifact         # pure offline analysis; the committed depth artifact makes the calibration reproducible without network
      requires_env: []                 # NONE — no credentials; the live probe uses PUBLIC read-only endpoints
      active: true
      ci_validatable: true             # fully validated in-gate with no secret — pure deterministic module over a committed artifact
      real_flow_note: "This capability carries NO edge claim and reaches NO revenue field. floor_feasibility REFUSES to solve for a stake when edge_fraction_per_trade <= 0 -- the project's current state -- and that refusal is exercised directly by a test. That is a guard for the CURRENT state, NOT a structural impossibility: fed a POSITIVE edge the function does produce a $/week figure. An adversarial reviewer demonstrated it could produce a FABRICATED one in the saturated-impact regime (impact_fraction is clamped at 1.0, which makes drag linear, so an edge above the saturated slope has no finite peak and the grid search returned its own upper bound dressed up as a ceiling -- reproduced at price 0.95 / edge 0.037 as a $112m/wk ceiling, inside NearCertaintyStrategy own operating envelope). That regime is now DETECTED and refused with an explicit model-artifact reason, pinned by tests at BOTH a high price and a low one. A re-review found the first version of the guard was only partial -- it derived the saturated slope as (1-flat)/flat, which assumes the impacted price asymptotes to the $1.00 cap and is only true for flat >= 0.5, letting the same unbounded case back in below price 0.49 (444 combinations found, squarely inside this repo's longshot-heavy median price of ~0.04). The slope is now MEASURED from the cost model by pricing a deliberately saturating order, which is correct at any price and cannot drift if the cost model or the impact clamp changes. The honest claim is that this module has no validated edge to size, NOT that it cannot emit a number."
      status: validated

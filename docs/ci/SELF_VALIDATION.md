...
SELF_VALIDATION:
  ...
  capabilities:
    ...
    - id: capacity_impact_analysis
      ...
      real_flow_note: "This capability carries NO edge claim and reaches NO revenue field. It CANNOT produce a $/week number: floor_feasibility explicitly refuses to solve for a stake when edge_fraction_per_trade <= 0, which is the project's current state, and that refusal is exercised directly by a test. The measured finding is that the shipped impact_coeff=0.5 over-charges the median book (implied coeff ~0.0) and under-charges the thin tail (up to 17.6), i.e. no single global coefficient is correct — reported as such rather than silently re-tuned to a new placeholder. scripts/capacity_probe.py performs READ-ONLY public market-data reads (Gamma + CLOB /book): no credentials, no orders, no venue writes."
      status: validated

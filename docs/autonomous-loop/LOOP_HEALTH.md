...
LOOP_HEALTH:
  project: LLM-Quant
  as_of: 2026-07-01
  last_run: 2026-06-30          # prior run (5th of 2026-06-30): factory run 18 — phantom-0.0-settlement side-effect-integrity fix (#104)
  last_deep_audit: 2026-07-01   # 6-Haiku skeptical scout sweep (data-parser / risk-safety / backtest-learning / security / frontend / artifact lenses) ran THIS factory run
  ...
  validation:                   # self-validation capability readiness — refresh every run from `check_self_validation.py --readiness`
    ...
    capabilities_total: 10      # unchanged this run (no new capability/credential; correctness/security/honesty hardening)
  ...
  this_run:
    changes_shipped: 3          # 3 code PRs (#108 resolution-by-id + order-book honesty; #109 §12 input bounds; #110 frontend honest empty-state) + this bookkeeping
    changes_abandoned: 0
    abandoned_reasons: []        # Rejected pre-build (not abandoned): WS empty-market_id-to-DB (low value — feed only has token_id), /learn/explain kwargs (low confidence — fixed method dict), risk+backtest scouts NOTHING-GENUINE (well-hardened), NearCertainty 72h/720h (OWNER F1). Deferred as documented next-run notes (out of scope, not dead-ends): paper_simulator._resolve_market same slug-vs-id class (degrades safely), _persist_resolution swallow (reporting-only, proven non-safety-critical this run).
    verify_cycle_failures: 0
    review_rejections: 0         # all 7 reviewers APPROVE/SOUND first pass (2 Sonnet each + 1 Opus safety auditor on the money-path #108). BOTH #108 Sonnet reviewers AND the Opus auditor independently REVERTED the one-line fix and confirmed the regression test fails on pre-fix code (0.0 == -50.0). 0 fix cycles.
    process_incidents: 0
    circuit_breaker_trips: 0
  rolling_7d:
    merged_prs: 66             # +4 this run (#108 + #109 + #110 + bookkeeping)
    reverts: 0
    readiness_attempts: 0
    readiness_rejected: 0
    recurring_failures: []       # OA-11 (Polymarket) + OA-15 (Kalshi) corpus + OA-16 (HuggingFace bypass) all owner/egress-scope. ALL egress 000 in-env again this run — confirmed owner-scope, not a loop wall. NOT trending `stuck`: convergence is owner-blocked, not rule-blocked, tracked across 4 owner actions.
    harness_proposals_open: 0
  signal: improving              # 19th datapoint: factory run, 1st of 2026-07-01. Found + fixed a HIGH-severity BUILDS!=WORKS that 5 prior skeptical scout sweeps missed: check_resolutions() looked markets up via get_market_by_slug(pos.market_id) but positions store the Gamma NUMERIC id (a DISTINCT field from slug), so a slug-filter query with a numeric id matched NOTHING -> NO position ever settled in prod -> the dominant binary-market loss path (resolution PnL, loss caps, kill switch, MTM reconcile) was silently DEAD. Every test passed because the fakes ignore their argument (a mock that accepts any input can't catch a wrong-key bug). The fix (#108: get_market_by_id + a regression test PROVEN to fail on pre-fix code) was NOT a scout find - it was the prior run's #104 Opus-auditor NEXT-RUN NOTE, investigated FIRST this run by observing the real code. Also shipped #109 (§12 bounds on the last 2 unbounded state-mutating request models) + #110 (frontend honest '-' not fake $0.00). `improving` (3 PRs vs prior 1, 0 abandoned/revert, a real high-severity defect closed); the adversarial gate earned its keep with 7 reviewers/0 fix cycles, and the Opus auditor independently validated the _persist_resolution deferral is SAFE (executor.positions is never rehydrated - load_positions_into_executor has 0 callers - and loss counters/kill-switch state persist independently BEFORE _persist_resolution). No DoD/floor box ticked; engine_pct stays 74. BINDING CONSTRAINT STILL OWNER-BLOCKED: real DoD/floor movement needs a real less-pinned OOS corpus + alpha, owner-action OA-16 (HuggingFace, preferred) / OA-13 / OA-11 - all egress 000 in-env. NOT churning/stuck, so no harness proposal; highest-value next action is the owner's, surfaced via notification. unmet=[]. NEXT-RUN NOTES: (1) paper_simulator._resolve_market has the same slug-vs-id call (degrades safely via a get_markets scan - a follow-up tidy, give it get_market_by_id); (2) _persist_resolution still swallows DB failures (reporting-only divergence, non-safety-critical this run); (3) confirm the Gamma ?id= filter param on the first real fetch (documented-but-unverified-live, egress-blocked).
```

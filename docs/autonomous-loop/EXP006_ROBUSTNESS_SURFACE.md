...
> **underpowered** (N < 100) **and** every one of them is **also F10-gate-fragile**. **No edge
> is claimed and none exists here.**
...
- **horizon** ∈ {6h, 12h, 24h\*}  (reversal horizon; **all ≤ the 24h default** so the engine's
  ...
  ≤ 1 day — applies fairly to every cell's gate)
...
| F11 `indistinguishable_from_zero` | 15 |
| F11 `insufficient_data` (N < 100 floor) | 27 |
| cells with positive net PnL | 14 |
| — of which **gate-F10-fragile** | **14 (all of them)** |
| — of which gate-F10-broad | **0** |
...
**The load-bearing pattern:** **no powered cell (N ≥ 100) is F11 `significant_positive`**,
while **18 powered cells are `significant_NEGATIVE`**. Every one of the 14 positive-PnL cells is
*doubly* disqualified — it is either underpowered (N < 100 → F11 `insufficient_data`) or F11
`indistinguishable_from_zero`, **and** it is F10-gate-fragile (concentrated); **zero** positive
cells are F10-broad. The positive region is entirely the high-threshold / 1h-window corner where
the fade takes too few trades to clear the pre-registered 100-event floor. Widening the window
(2h/6h) recruits more spikes, but they do not revert — the two-way cost hurdle then drives the
total significantly negative.

> **F10 column note.** The `F10 gate-fragile` column is the SAME F10 the gate uses for
> `is_validated_edge` (`res.f10_gate_ok`: horizon-axis excluded while single-valued,
> size-robustness folded in), **not** the raw horizon-inclusive `regime.fragile`. It shows
> `–` where F10 is not meaningfully assessed (no trades, or a non-positive aggregate that F11
> already rejects), `True` for a concentrated positive edge, `False` for a broad one — so it can
> never read `True` next to a validated cell.

## Full surface (all 60 cells — report-all, select-none)

| thr | win | hor | N | net PnL | hit | F11 | F10 gate-fragile | valid |
|-----|-----|-----|---|---------|-----|-----|------------------|-------|
| 0.05 | 30m | 6h | 0 | 0 | – | insufficient | – | False |
| 0.05 | 30m | 12h | 0 | 0 | – | insufficient | – | False |
| 0.05 | 30m | 24h | 0 | 0 | – | insufficient | – | False |
| 0.05 | 1h | 6h | 152 | −1,296 | 0.33 | significant_neg | – | False |
| 0.05 | 1h | 12h | 152 | −1,080 | 0.37 | significant_neg | – | False |
| 0.05 | 1h | 24h | 152 | −1,355 | 0.38 | significant_neg | – | False |
| 0.05 | 2h | 6h | 169 | −1,690 | 0.36 | significant_neg | – | False |
| 0.05 | 2h | 12h | 169 | −2,004 | 0.34 | significant_neg | – | False |
| 0.05 | 2h | 24h | 169 | −1,687 | 0.37 | significant_neg | – | False |
| 0.05 | 6h | 6h | 177 | −1,907 | 0.29 | significant_neg | – | False |
| 0.05 | 6h | 12h | 177 | −2,257 | 0.27 | significant_neg | – | False |
| 0.05 | 6h | 24h | 177 | −1,959 | 0.32 | significant_neg | – | False |
| 0.08 | 30m | 6h | 0 | 0 | – | insufficient | – | False |
| 0.08 | 30m | 12h | 0 | 0 | – | insufficient | – | False |
| 0.08 | 30m | 24h | 0 | 0 | – | insufficient | – | False |
| 0.08 | 1h | 6h | 121 | −93 | 0.45 | indistinguishable | – | False |
| 0.08 | 1h | 12h | 122 | −109 | 0.49 | indistinguishable | – | False |
| 0.08 | 1h | 24h | 122 | −69 | 0.52 | indistinguishable | – | False |
| 0.08 | 2h | 6h | 142 | −1,305 | 0.39 | significant_neg | – | False |
| 0.08 | 2h | 12h | 143 | −1,631 | 0.38 | significant_neg | – | False |
| 0.08 | 2h | 24h | 143 | −1,462 | 0.37 | significant_neg | – | False |
| 0.08 | 6h | 6h | 153 | −1,346 | 0.37 | significant_neg | – | False |
| 0.08 | 6h | 12h | 154 | −1,725 | 0.35 | significant_neg | – | False |
| 0.08 | 6h | 24h | 154 | −1,550 | 0.36 | significant_neg | – | False |
...
| 0.10 | 2h | 6h | 128 | −624 | 0.47 | indistinguishable | – | False |
| 0.10 | 2h | 12h | 129 | −847 | 0.46 | indistinguishable | – | False |
| 0.10 | 2h | 24h | 129 | −825 | 0.46 | indistinguishable | – | False |
| 0.10 | 6h | 6h | 141 | −816 | 0.47 | significant_neg | – | False |
| 0.10 | 6h | 12h | 142 | −1,181 | 0.44 | significant_neg | – | False |
| 0.10 | 6h | 24h | 142 | −1,263 | 0.37 | significant_neg | – | False |
...
| 0.15 | 2h | 6h | 104 | −381 | 0.54 | indistinguishable | – | False |
| 0.15 | 2h | 12h | 105 | −236 | 0.50 | indistinguishable | – | False |
| 0.15 | 2h | 24h | 105 | −208 | 0.50 | indistinguishable | – | False |
| 0.15 | 6h | 6h | 117 | −543 | 0.50 | indistinguishable | – | False |
| 0.15 | 6h | 12h | 118 | −227 | 0.54 | indistinguishable | – | False |
| 0.15 | 6h | 24h | 118 | −257 | 0.51 | indistinguishable | – | False |
...
| 0.20 | 1h | 24h | 57 | −64 | 0.47 | insufficient | – | False |
...
2. **No cell is even close to validating.** No powered cell reaches F11 `significant_positive`;
   the positive point estimates are either underpowered (N < 100) or F11
   `indistinguishable_from_zero`, and each is *additionally* F10-gate-fragile (concentrated in
   one confidence band / category). So a positive cell fails on **two** independent gates, not
   one — there is no "almost an edge" region to chase.

3. **One underpowered hypothesis, flagged NOT claimed.** The high-threshold / 1h-window corner
   ...
   *magnitude-stratum* carve-out (post-hoc — `|peak−baseline|` is not knowable at decision
   ...
   underpowered** (N = 56–96, all below the 100-event floor → F11 `insufficient_data`) and every
   such positive cell is also F10-gate-fragile. It is therefore **a candidate for a FRESH,
   pre-registered OOS on new/larger data — NOT an edge, and not tradeable on this evidence.**
   Flagged here as an EXP-006b candidate (distinct from the EXP-007 momentum idea); recorded in
   the companion bookkeeping PR's ROADMAP / RESEARCH_MEMORY updates.

4. **No selection, no revenue field.** The surface computes a conservative chance-green

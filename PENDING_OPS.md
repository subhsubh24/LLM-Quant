...
OWNER_ACTIONS:
  items:
    ...
    - id: OA-15
      title: "Kalshi live-contract VERIFIED via the real-oos lane — found a price-history endpoint bug; fix is loop-buildable (A3)"
      priority: low
      status: in_progress
      why: "VERIFIED (2026-07-02, via the real-oos-validation lane running on a permitted host — GitHub runner + local): Kalshi market DISCOVERY works (real settled tickers returned), but the per-market price-history fetch **404s** — the fetcher calls `/trade-api/v2/markets/{ticker}/history`, which does not exist. So it returns 0 leakage-safe records (the lane reports Kalshi N/A, Polymarket flows). This is exactly the live-contract confirmation OA-15 existed for. NOT an owner action — the fix is loop-buildable (see below); no credentials needed (public data)."
...
| OA-15 | Kalshi live-contract VERIFIED (price-history endpoint 404 bug found; fix loop-buildable, A3) | ⚪ low | in progress |
